...
#define LOCTEXT_NAMESPACE "SThinkGraphEdge"

void SThinkGraphEdge::Construct(const FArguments& InArgs, UThinkGraphEdNodeEdge* InNode)
{
	check(InNode)
	GraphNode = InNode;
...
bool SThinkGraphEdge::RequiresSecondPassLayout() const
{
	return true;
}

void SThinkGraphEdge::PerformSecondPassLayout(const TMap<UObject*, TSharedRef<SNode>>& InNodeToWidgetLookup) const
{
	// CG_EDITOR_LOG(Verbose, TEXT("SThinkGraphEdge::PerformSecondPassLayout"));
	const UThinkGraphEdNodeEdge* EdgeNode = CastChecked<UThinkGraphEdNodeEdge>(GraphNode);
	...
	// UThinkGraphEdNode* Start = EdgeNode->GetStartNodeAsGraphNode();
	UThinkGraphEdNode* Start = EdgeNode->GetStartNodeAsBase();
	UThinkGraphEdNode* End = EdgeNode->GetEndNode();
...
void SThinkGraphEdge::UpdateGraphNode()
{
	InputPins.Empty();
	OutputPins.Empty();
	...
	GetOrAddSlot(ENodeZone::Center)
		...
		[
			...
			[
				...
				.ColorAndOpacity(this, &SThinkGraphEdge::GetEdgeColor)
			]
			+ SOverlay::Slot()
			[
				SNew(SImage)
				.Image(this, &SThinkGraphEdge::GetEdgeImage)
				.Visibility(this, &SThinkGraphEdge::GetEdgeImageVisibility)
			]
			...
			[
				...
				[
					...
					.Visibility(this, &SThinkGraphEdge::GetEdgeTitleVisibility)
					.Font(FCoreStyle::GetDefaultFontStyle("Regular", 12))
					.Text(NodeTitle.Get(), &SNodeTitle::GetHeadTitle)
					.OnTextCommitted(this, &SThinkGraphEdge::OnNameTextCommited)
				]
				+ SVerticalBox::Slot()
				.AutoHeight()
				[
...
void SThinkGraphEdge::PositionBetweenTwoNodesWithOffset(const FGeometry& StartGeom, const FGeometry& EndGeom, int32 NodeIndex, int32 MaxNodes) const
{
...
void SThinkGraphEdge::OnNameTextCommited(const FText& InText, const ETextCommit::Type CommitInfo)
{
	SGraphNode::OnNameTextCommited(InText, CommitInfo);

	UThinkGraphEdNodeEdge* MyNode = CastChecked<UThinkGraphEdNodeEdge>(GraphNode);
...
FSlateColor SThinkGraphEdge::GetEdgeColor() const
{
	UThinkGraphEdNodeEdge* EdgeNode = CastChecked<UThinkGraphEdNodeEdge>(GraphNode);
...
const FSlateBrush* SThinkGraphEdge::GetEdgeImage() const
{
	...
	if (UTexture2D* Texture = CachedGraphEdge->GetCachedIconTexture())
	{
		...
		// // const float IconSize = FHBThinkGraphUtils::GetPluginProjectSettings()->IconSize;
...
EVisibility SThinkGraphEdge::GetEdgeImageVisibility() const
{
	return EVisibility::Visible;
}

EVisibility SThinkGraphEdge::GetEdgeTitleVisibility() const
{
