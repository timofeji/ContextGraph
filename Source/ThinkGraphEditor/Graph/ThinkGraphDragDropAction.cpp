...
#include "ThinkGraphDragDropAction.h"
...
FThinkGraphDragDropAction::FThinkGraphDragDropAction(const TSharedRef<SGraphPanel>& GraphPanel, TArray<FGraphPinHandle>& DraggedPins)
...
TSharedRef<FThinkGraphDragDropAction> FThinkGraphDragDropAction::New(const TSharedRef<SGraphPanel>& GraphPanel, TArray<FGraphPinHandle>& InStartingPins)
{
	TSharedRef<FThinkGraphDragDropAction> Operation = MakeShareable(new FThinkGraphDragDropAction(GraphPanel, InStartingPins));
	Operation->Construct();
	return Operation;
}

void FThinkGraphDragDropAction::OnDrop(const bool bDropWasHandled, const FPointerEvent& MouseEvent)
{
	GraphPanel->OnStopMakingConnection();
	FGraphEditorDragDropAction::OnDrop(bDropWasHandled, MouseEvent);
}

void FThinkGraphDragDropAction::HoverTargetChanged()
{
	...
	else if (const UThinkGraphEdNode* TargetNode = Cast<UThinkGraphEdNode>(GetHoveredNode()))
	{
...
void FThinkGraphDragDropAction::OnDragged(const FDragDropEvent& DragDropEvent)
{
...
FReply FThinkGraphDragDropAction::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FThinkGraphDragDropAction::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FThinkGraphDragDropAction::DroppedOnPanel(const TSharedRef<SWidget>& Panel, const FVector2D ScreenPosition, const FVector2D GraphPosition, UEdGraph& Graph)
{
	// Gather any source drag pins
...
void FThinkGraphDragDropAction::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
...
void FThinkGraphDragDropAction::AppendConnectionResponsesForHoveredPin(const UEdGraphPin* TargetPin, TArray<FPinConnectionResponse>& OutResponses)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(ValidSourcePins);
...
void FThinkGraphDragDropAction::AppendConnectionResponsesForHoveredNode(const UThinkGraphEdNode* TargetNode, TArray<FPinConnectionResponse>& OutResponses)
{
	...
	for (const UEdGraphPin* StartingPin : StartingPins)
	{
		...
		{
			Response = FPinConnectionResponse(CONNECT_RESPONSE_DISALLOW, LOCTEXT("PinError", "Not a valid Think Graph Node"));
		}
...
void FThinkGraphDragDropAction::AppendConnectionResponsesForHoveredGraph(const UEdGraph* HoverGraph, TArray<FPinConnectionResponse>& OutResponses)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(ValidSourcePins);
