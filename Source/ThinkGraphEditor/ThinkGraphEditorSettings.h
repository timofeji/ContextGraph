#pragma once
#include "ThinkGraphEditorSettings.generated.h"
...
UENUM()
enum class EThinkGraphDrawPinTypes : uint8
{
	/** Setup Event */
...
UCLASS(Config = Game,
	defaultconfig)
class THINKGRAPHEDITOR_API UThinkGraphEditorSettings : public UObject
{
	GENERATED_BODY()

public:
	UThinkGraphEditorSettings();
	...
	// EHBThinkGraphDrawPinTypes DrawPinTypes = EHBThinkGraphDrawPinTypes::Both;
	 ...
	 *
	 * This is set by default to an internal DataTables (that you can find in `/HBThinkGraph/Xelu_Icons/`) that setup texture icons for every keyboard and gamepad key.
	 *
	 ...
	 */
	// UPROPERTY(config, EditDefaultsOnly, Category = "Icons", meta = (RequiredAssetDataTags = "RowStructure=HBThinkGraphInputsMetaData"))
	...
	// EHBThinkGraphIconType IconPreference = EHBThinkGraphIconType::Keyboard;

	/** Size of Icons when drawing edges (transitions) in HBThinkGraphs */
	UPROPERTY(config,
		EditAnywhere,
		Category = "Icons",
